...
/*   Updated: 2025/01/19 00:37:56 by vbui             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsing_args(int argc, char **argv, t_p *p)
{
	if ((argc == 5 || argc == 6) && ft_numerik(argv, 1))
	{
		...
		if (p->a.total <= 0 || p->a.die <= 0 || p->a.eat <= 0
			|| p->a.sleep <= 0)
			return (ft_byebye("Philo need 1 or more\n"));
		if (p->a.die > 1000 || p->a.eat > 1000 || p->a.sleep > 1000)
			return (ft_byebye("time to die/eat/sleep max = 1000\n"));
		if (p->a.total > MAX_PHILOS)
			return (ft_byebye("Max philo 200\n"));
		return (1);
	}
	return (0);
}
