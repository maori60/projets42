...
/*   Updated: 2025/01/17 23:42:26 by vbui             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_atoi(const char *str)
{
	...
	while (str[i])
	{
		// Vérifie les dépassements potentiels
		if (sum > 2147483647)
			return (-1);
		sum = (sum * 10) + (str[i] - '0');
		i++;
	}
	// Vérifie encore après le calcul complet
	if (sum > 2147483647)
		return (-1);
	return ((int)sum);
}
...
int	ft_numerik(char **argv, int j)
{
	...
	{
		i = 0;
		// Vérifie que chaque caractère est un chiffre et que la longueur est correcte
		while (argv[j][i])
		{
		...
		// Vérifie la validité de la valeur entière (pas de dépassement, pas négatif)
...
int	parsing_args(int argc, char **argv, t_p *p)
{
	if ((argc == 5 || argc == 6) && ft_numerik(argv, 1))
	{
		...
		// Validation des bornes minimales
		if (p->a.total <= 0 || p->a.die <= 0 || p->a.eat <= 0 || p->a.sleep <= 0)
			return (0);
		// Limites maximales pour éviter des valeurs absurdes
